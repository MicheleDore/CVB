...
const metaDebate = (req, res)=>{
    const getComments = ` SELECT comments.content, DATE_FORMAT(publication_time, "%d/%m/%Y     %H:%i:%s") AS publication_time, users.nickname FROM comments JOIN users ON comments.user_id=users.id WHERE choice_id = ? ORDER BY comments.id DESC`
